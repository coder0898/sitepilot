...
export function Button({ as: Component = "button", variant = "primary", size = "md", className, type, children, loading = false, disabled = false, ...props }) {
  return (
    <Component
      type={Component === "button" ? (type || "button") : type}
      className={cn("inline-flex shrink-0 cursor-pointer items-center justify-center gap-2 font-bold transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 disabled:pointer-events-none disabled:opacity-50", variants[variant], sizes[size], className)}
      {...props}
      disabled={Component === "button" ? (loading || disabled) : undefined}
      aria-busy={loading || undefined}
    >
      {loading && <LoaderCircle aria-hidden="true" className="animate-spin" size={17} />}
      {children}
    </Component>
  );
}
