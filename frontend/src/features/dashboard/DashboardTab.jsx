...
export function DashboardTab({ tab, loading, data, user, action, onOpenProject, onRefresh }) {
  if (loading) {
  ...
  return <Component key={`${tab}:${identityKey}`} data={data} user={user} action={action} onOpenProject={onOpenProject} onRefresh={onRefresh} />;
}
