...
export function CreateUserModal({ create, catalog, manageableRoles, onClose }) {
  ...
  return <Modal title="Add team member" subtitle="Pre-register their role and details now - they activate it themselves with one click on Sign in with Google, no password to set up." onClose={onClose} className="sm:max-w-3xl"><form className="grid gap-5" onSubmit={create}><CapabilityPreview definition={definition}/><section className="grid gap-4 rounded-[22px] border border-slate-200 bg-white p-4 sm:grid-cols-2 sm:p-5"><Field label="Full name"><Input name="name" autoComplete="name" placeholder="e.g. Ananya Shah" required/></Field><Field label="Email login" hint="Must match the Google account they'll sign in with."><Input name="email" type="email" autoComplete="email" placeholder="name@company.com" required/></Field><Field label="Mobile number" hint="Use international format for future WhatsApp identity."><Input name="phone" inputMode="tel" placeholder="+919876543210"/></Field><Field label="System role"><Select name="role" value={selectedRole} onChange={event => setSelectedRole(event.target.value)} required>{manageableRoles.map(role => <option key={role} value={role}>{roles[role]}</option>)}</Select></Field><Field label="Employee code"><Input name="employee_code" placeholder="EMP-00124" required/></Field><Field label="Designation"><Input name="designation" placeholder="Project Manager" required/></Field><Field label="Department"><Input name="department" placeholder="Projects / Operations"/></Field></section><FormActions><Button variant="secondary" onClick={onClose}>Cancel</Button><Button type="submit"><UserPlus size={18}/>Add to roster</Button></FormActions></form></Modal>;
}
