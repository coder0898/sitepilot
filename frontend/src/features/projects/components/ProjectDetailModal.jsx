...
export function ProjectDetailModal({ projectId, references, templates = [], user, onClose, onEdit, onChanged, onDeleted }) {
  ...
  const detailTabs = [["overview", CalendarDays, "Overview"], ...(!["supervisor", "internal_employee"].includes(user.role) ? [["template-review", Layers3, "Template Review"]] : []), ["team", UsersRound, "Team"], ["activity", Activity, "Activity"]];
  useEffect(() => { if (["supervisor", "internal_employee"].includes(user.role) && tab === "template-review") setTab("overview"); }, [tab, user.role]);
