...
export function ProjectManualGateModal({ projectId, onClose, onCreated }) {
  ...
  return <Modal title="Add manual external approval" subtitle="Create a project-only approval. The published template remains unchanged." onClose={() => { if (!saving) onClose(); }} className="sm:max-w-3xl">
    <form className="grid gap-5" onSubmit={submit}>
      <div className="flex items-center justify-between gap-3 rounded-2xl border border-blue-200 bg-blue-50 p-4"><div><p className="text-sm font-black text-blue-950">Project-only scope</p><p className="mt-1 text-xs text-blue-700">The backend assigns a unique manual gate code and the Project Manager as owner.</p></div><Pill tone="blue">Manual source</Pill></div>
