...
export function ProjectsPage({ user, action }) {
  ...
  async function load() {
    setLoading(true);
    try { const [items, refs] = await Promise.all([projectsApi.list(), projectsApi.references()]); setProjects(items); setReferences(refs); }
  ...
  async function saveProject(payload) {
    ...
    const result = await action(() => editing ? projectsApi.update(formProject.id, payload) : projectsApi.create(payload), editing ? "Project updated" : "Draft project created", { refresh: false });
    setSaving(false);
    if (result.ok) { setFormProject(undefined); await load(); }
  }

  return <section className="grid gap-5">
    <header className="flex flex-col gap-4 rounded-[24px] border border-slate-200/80 bg-white p-5 shadow-[0_16px_50px_rgba(15,23,42,.06)] sm:p-6 lg:flex-row lg:items-end lg:justify-between"><div><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.2em] text-blue-700"><BriefcaseBusiness size={15}/> Project portfolio</div><h2 className="mt-2 text-2xl font-black tracking-[-.035em] text-slate-950 sm:text-3xl">Projects under control</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Create accountable project records first. Templates, schedules and field execution attach only after the draft setup is complete.</p></div>{user.role === "admin" && <Button size="lg" onClick={() => setFormProject(null)}><Plus size={18}/> New project</Button>}</header>
