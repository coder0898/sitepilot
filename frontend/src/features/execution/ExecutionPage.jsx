...
import { Activity, AlertTriangle, Bell, BriefcaseBusiness, CalendarDays, CheckCircle2, ClipboardList, Clock3, LayoutTemplate, Plus, RefreshCw, UserRound, Users } from "lucide-react";
import { executionApi } from "../../api/executionApi";
import { ConfirmModal, Pill } from "../../components/ui";
import { DayColumn, EmptySmall, ExecutionMetric as Metric, NotificationDeliveryHistory, TeamMember } from "./components/ExecutionOverview";
...
export function ExecutionPage({ user, action }) {
  ...
  async function retryNotification(id) { await perform(() => executionApi.retryNotification(id), "Notification queued for mock retry"); }
  ...
  return (
    <div className="exec-dashboard grid min-w-0 max-w-full gap-4 text-slate-900">
      ...
      {view === "templates" && user.role === "super_admin" ? <TemplateView templates={data.templates} open={() => setForm("template")}/> : <>
        <section className="exec-metrics grid grid-cols-6 gap-3 max-[1180px]:grid-cols-3 max-[720px]:grid-cols-2 max-[420px]:grid-cols-1">
          ...
          <Metric icon={<Bell/>} label="Notifications" value={notifications.length} helper={`${notifications.filter(note => note.status === "failed").length} failed`} tone="red"/>
        ...
        {project ? <>
          ...
          <div className="exec-primary-grid grid min-w-0 grid-cols-[minmax(0,2.1fr)_minmax(270px,0.75fr)] items-start gap-4 max-[1180px]:grid-cols-1">
            ...
            <section className="exec-activity-card min-w-0 rounded-2xl border border-slate-200 bg-white shadow-[0_10px_30px_rgba(26,47,78,0.06)] [&>header]:flex [&>header]:items-center [&>header]:justify-between [&>header]:gap-3 [&>header]:border-b [&>header]:border-slate-200 [&>header]:px-5 [&>header]:py-4 [&_h3]:m-0 [&_h3]:font-serif [&_h3]:text-lg [&>div]:px-4 [&>div>article]:grid [&>div>article]:grid-cols-[auto_minmax(0,1fr)_auto] [&>div>article]:items-center [&>div>article]:gap-3 [&>div>article]:border-b [&>div>article]:border-slate-100 [&>div>article]:py-3 [&_article>div]:min-w-0 [&_article>div]:grid [&_article_strong]:truncate [&_article_strong]:text-sm [&_article_p]:m-0 [&_article_p]:text-xs [&_article_p]:text-slate-500 [&_article_small]:text-[10px] [&_article_small]:text-slate-400 flex max-h-[480px] min-h-0 flex-col overflow-hidden md:max-h-[460px] xl:max-h-[460px]">
              <header className="shrink-0"><h3>Notification Activity</h3><span>Mock delivery</span></header>
              <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain [scrollbar-gutter:stable]">{notifications.map(note => <article key={note.id}><span className={`grid size-9 place-items-center rounded-xl [&_svg]:size-4 ${note.status === "failed" ? "bg-rose-50 text-rose-700" : note.status === "delivered" ? "bg-emerald-50 text-emerald-700" : "bg-blue-50 text-blue-700"}`}><Bell/></span><div><strong>{note.notification_type === "material_reminder" ? "Material reminder" : note.recipient_name}</strong><p>{note.taskTitle}</p><small>{note.recipient_name} · {note.recipient_type} · {note.phone || "Phone missing"}</small></div><Pill tone={deliveryTone(note.status)}>{note.status}</Pill></article>)}{!notifications.length && <EmptySmall text="Task notifications will appear here."/>}</div>
            </section>
          </div>
          ...
          <NotificationDeliveryHistory notifications={notifications} filter={notificationFilter} setFilter={setNotificationFilter} canRetry={canManage} retry={retryNotification}/>
