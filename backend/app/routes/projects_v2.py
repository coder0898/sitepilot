...
def can_edit(db: Session, project: V2Project, actor: User) -> bool:
    return actor.role == UserRole.admin or (actor.role == UserRole.project_manager and has_membership(db, project.id, actor, {"project_manager"}))
...
def reference_data(actor: User = Depends(current_user), db: Session = Depends(get_db)):
    can_assign_pm = actor.role == UserRole.admin
    can_assign_supervisor = actor.role in {UserRole.admin, UserRole.project_manager}
    can_assign_support = actor.role in {UserRole.admin, UserRole.project_manager, UserRole.supervisor}
    return {
...
@router.post("")
def create_project(payload: ProjectCreateIn, actor: User = Depends(require_roles(UserRole.admin)), db: Session = Depends(get_db)):
    code = clean_required(payload.code, "Project code").upper()
    ...
    project = V2Project(code=code, name=clean_required(payload.name, "Project name"), client_name=clean_required(payload.client_name, "Client name"), site_address=clean_required(payload.site_address, "Site address"), description=clean_optional(payload.description), start_date=payload.start_date, target_handover_date=payload.target_handover_date, status="draft", created_by=actor.id)
    db.add(project)
    try:
        db.flush()
        add_audit(db, actor, project, "PROJECT_CREATED", "Draft project created.", after=project_snapshot(project))
        reason = clean_optional(payload.assignment_reason) or "Initial project setup."
        if payload.project_manager_employee_id:
            assign_membership(db, project, payload.project_manager_employee_id, "project_manager", reason, actor)
        if payload.supervisor_employee_id:
            assign_membership(db, project, payload.supervisor_employee_id, "site_supervisor", reason, actor)
        db.commit()
